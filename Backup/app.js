...
const UI = (() => {
  ...
  const showTab = (tabName) => {
    const validTabs = ['dashboard', 'transakcije', 'zaposleni', 'inventar', 'fakture', 'istorija', 'izvestaj'];
...
const Receipts = (() => {
  ...
  const getByStatus = (status) => {
    const receipts = getAll().filter(r => r.status === status);
    if (status === 'unpaid') {
      return receipts.sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate));
    } else {
      return receipts.sort((a, b) => new Date(b.dateReceived) - new Date(a.dateReceived));
    }
  };

  const getUnpaidSorted = (sortBy) => {
    const receipts = getAll().filter(r => r.status === 'unpaid');
    switch (sortBy) {
      case 'name':
        return receipts.sort((a, b) => a.vendor.localeCompare(b.vendor));
      case 'amount':
  ...
  return { add, update, markAsPaid, remove, getAll, getById, getByStatus, getUnpaidSorted, getOverdue, getDaysUntilDue, getCategories, getDefaultCategories, addCategory, removeCategory };
})();
...
function showSubtab(name) {
  UI.showSubtab(name);
  if (name === 'nepaid-receipts' || name === 'paid-receipts') {
    renderReceiptsList();
  }
}
...
function renderReceiptsList() {
  ...
  const unpaidTotalContainer = document.getElementById('unpaid-receipts-total');
  const paidTotalContainer = document.getElementById('paid-receipts-total');

  const sortBy = document.getElementById('unpaid-sort-select')?.value || 'dueDate';
  const unpaidReceipts = Receipts.getUnpaidSorted(sortBy);
  const paidReceipts = Receipts.getByStatus('paid');

  // Calculate unpaid total
  const unpaidTotal = unpaidReceipts.reduce((sum, r) => sum + (r.amount || 0), 0);
  ...
  if (unpaidReceipts.length === 0) {
    ...
    unpaidTotalContainer.style.display = 'none';
  } else {
    ...
    document.getElementById('unpaid-total-amount').textContent = UI.fmt(unpaidTotal);
  }

  // Calculate paid total
  const paidTotal = paidReceipts.reduce((sum, r) => sum + (r.amount || 0), 0);
  ...
  if (paidReceipts.length === 0) {
    ...
    paidTotalContainer.style.display = 'none';
  } else {
    ...
    paidTotalContainer.style.display = 'block';
    document.getElementById('paid-total-amount').textContent = UI.fmt(paidTotal);
  }
}
